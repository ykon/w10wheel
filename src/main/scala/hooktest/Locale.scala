...
object Locale {
    ...
    def convLang(lang: String, msg: String): String = {
        lang match {
            case DataID.English => msg
            case DataID.Japanese => LocaleData.get_ja(msg)
            case _ => {
                logger.warn(s"Not supported language: $lang")
                msg
            }
        }
    }
...
private object LocaleData {
    private val JapaneseMessage = Map(
    ...
    def get_ja(msg: String): String = {
        JapaneseMessage.get(msg) match {
            case Some(ja_msg) => ja_msg
            case None => msg
        }
    }
}